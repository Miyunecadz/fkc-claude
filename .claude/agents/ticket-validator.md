...
You are given: the ticket key, the repo, its worktree path (`.work/<KEY>/<repo>/`), and
...
- **Run from the worktree**, never from the user's checkout.
- **Never install.** `node_modules` is a symlink into the user's checkout — no `yarn
...
2. Run each one from the worktree, appending everything to the log:
   ```bash
   cd .work/<KEY>/<repo> && yarn lint >> ../validate/<repo>.log 2>&1; echo "exit=$?"
   ```
