...
enabled: true
event: bash
pattern: '(^|[;&|]\s*)git(\s+-{1,2}[A-Za-z][A-Za-z-]*(=\S+)?(\s+[^\s-]\S*)?)*\s+commit\b'
severity: warn
stacks: [all]
match: argv
surface: agent
...
**Surface: `agent` only.** Dispatching a reviewer agent needs a model, which a pre-commit hook has no business invoking — it would make every commit slow and non-deterministic.
